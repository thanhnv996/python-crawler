            ...
            for link in provinceATag:
                ...
                try:
                    while page <= numberOfPage:
                        ...
                        # break  #skip for one page
                    logger.info('Writing data to database...')
                    df.reset_index()
                    df.to_sql('areas', db_engine(), if_exists='append')
                    logger.info('Write data to database has done!')

                except Exception as e:
                    logger.error('Error with getProvinceOnePage: ' + str(e))
                # break  # skip for one province

        except Exception as e:
            logger.error('Error with link in provinceATag: ' + str(e))
